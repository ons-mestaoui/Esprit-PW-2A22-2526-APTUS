...
class CVC
{
    ...
    public function translateCV(array $cvData, string $targetLang): array {
        $prompt = "Tu es un traducteur expert. Traduis ce CV JSON en $targetLang.
RÈGLES ABSOLUES :
1. Retourne EXACTEMENT la même structure JSON avec les MÊMES clés.
2. Ne traduis JAMAIS les clés JSON (nomComplet, titrePoste, adresse, resume, experience, education, skills, languages, labels, role, company, dates, achievements, text, cat, degree, school, lang, level restent inchangés).
3. Traduis UNIQUEMENT les valeurs textuelles : nomComplet, titrePoste, adresse, resume, experience[].role, experience[].achievements[].text, education[].degree, skills[], languages[].lang, et les valeurs dans labels.
4. Garde INCHANGÉS : noms d'entreprises (company), noms d'établissements (school), dates (ex: '2020-2023'), niveaux de langue (B1, B2, C1, C2, A1, A2), valeurs du champ 'cat'.
5. Conserve EXACTEMENT la même structure imbriquée pour experience, education, skills, languages.
6. Retourne un objet JSON valide et complet contenant TOUS les champs présents dans l'entrée.";
