...
class TuteurDashboardController
{
    ...
    public function updateProgression($id_formation, $id_user, $progression)
    {
        ...
        try {
            $stmt = $db->prepare("UPDATE inscription SET progression = :p, statut = :s WHERE id_formation = :f AND id_utilisateur = :u");
            ...
            if ($success && $progression >= 100) {
                ...
                $titre = $formation['titre'] ?? 'une formation';
    ...
    public function getRecentAIAlerts($id_tuteur)
    {
        ...
        try {
            ...
            // Logique d'alerte : Si confusion (fearful), tristesse (sad) ou colère (angry) > 3 occurrences en 15 min
            foreach ($grouped as $data) {
                $counts = array_count_values($data['emotions']);
                $confusionCount = ($counts['fearful'] ?? 0) + ($counts['sad'] ?? 0) + ($counts['angry'] ?? 0);
