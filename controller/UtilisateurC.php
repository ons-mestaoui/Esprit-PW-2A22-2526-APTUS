...
class UtilisateurC {
    ...
    public function deleteUtilisateur($id) {
        $db = config::getConnexion();
        try {
            // Fetch role so we only touch the relevant FK branch
            $stmt = $db->prepare("SELECT role FROM utilisateur WHERE id_utilisateur = :id");
            $stmt->execute(['id' => $id]);
            $user = $stmt->fetch();
            $role = $user['role'] ?? '';

            // Candidat FK (candidat.id_candidat → utilisateur.id_utilisateur)
            // Must delete candidat's dependents before the candidat row, then the FK is satisfied
            if ($role === 'Candidat') {
                ...
                $db->prepare("DELETE FROM inscription WHERE id_utilisateur = :id")->execute(['id' => $id]);
                $db->prepare("DELETE FROM candidat WHERE id_candidat = :id")->execute(['id' => $id]);
            }

            // Entreprise FK (entreprise.id_entreprise → utilisateur.id_utilisateur)
            // Must delete offreemploi (and its candidatures) before the entreprise row
            if ($role === 'Entreprise') {
            ...
            // Admin: remove admin-specific data, then the admin role row
            if ($role === 'Admin') {
            ...
            // Tuteur: detach from formations (don't delete them), then the tuteur role row
            if ($role === 'Tuteur') {
            ...
            // Common: profil is linked to utilisateur regardless of role
            $db->prepare("DELETE FROM profil WHERE id_utilisateur = :id")->execute(['id' => $id]);

            // Finally remove the utilisateur row itself
            $db->prepare("DELETE FROM utilisateur WHERE id_utilisateur = :id")->execute(['id' => $id]);
        } catch (Exception $e) {
    ...
    public function getPreferences($id) {
        ...
        $defaults = [
            ...
            'privacy_search' => true,
            'notif_jobs' => true,
            'notif_candidatures' => true,
            'notif_formations' => false,
