...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'delete_account') {
        ...
        $utilisateurC->updatePreferences($id, [
            'notif_jobs'          => isset($_POST['notif_jobs']),
            'notif_candidatures'  => isset($_POST['notif_candidatures']),
            'notif_formations'    => isset($_POST['notif_formations']),
            'notif_newsletter'    => isset($_POST['notif_newsletter']),
        ]);
        $prefs = $utilisateurC->getPreferences($id);
        $successMsg = "Préférences de notifications enregistrées.";
...
<div class="settings-section" id="tab-notifications">
  <form method="POST" action="">
    ...
    <div class="settings-card">
      ...
      <?php
      $notifDefs = [
        'notif_jobs'         => ['label' => 'Nouveaux postes correspondants', 'hint' => 'Recevez un email quand un poste correspond à votre profil'],
        'notif_candidatures' => ['label' => 'Mises à jour des candidatures',  'hint' => 'Statut de vos candidatures et retours des entreprises'],
        'notif_formations'   => ['label' => 'Nouvelles formations',            'hint' => 'Soyez informé des nouvelles formations et certifications'],
        'notif_newsletter'   => ['label' => 'Newsletter Aptus',               'hint' => 'Actualités, conseils carrière et tendances du marché'],
      ];
      foreach ($notifDefs as $key => $def):
        $checked = !empty($prefs[$key]);
      ?>
      <div class="setting-row">
        <div class="setting-row__info">
          <div class="setting-row__label"><?= htmlspecialchars($def['label']) ?></div>
          <div class="setting-row__hint"><?= htmlspecialchars($def['hint']) ?></div>
        </div>
        <label class="toggle-switch <?= $checked ? 'active' : '' ?>" onclick="this.classList.toggle('active'); this.querySelector('input').checked = this.classList.contains('active');">
          <input type="checkbox" name="<?= $key ?>" value="1" <?= $checked ? 'checked' : '' ?> style="display:none;">
        </label>
      </div>
      <?php endforeach; ?>
    </div>
    <button type="submit" class="btn btn-primary" style="width:fit-content; margin-top:var(--space-3);">
      <i data-lucide="save" style="width:16px;height:16px;"></i> Enregistrer les notifications
    </button>
  </form>
</div>
