...
if (empty($room_url) || $room_url === '#' || trim($room_url) === '%23' || strpos($room_url, 'meet.jit.si') === false) {
    $safe_name = "AptusSession_" . $id_formation . "_" . substr(md5($id_formation . "aptus_2024"), 0, 8);
    $room_url = "https://meet.jit.si/" . $safe_name;
}

// Nettoyage du nom de la salle
$room_name = str_replace('https://meet.jit.si/', '', $room_url);
$room_name = explode('?', $room_name)[0]; 
        ...
        <?php if ($role !== 'tuteur'): ?>
            <div title="Analyse cognitive IA en Edge Computing" style="font-size:0.82rem;">
                <i data-lucide="brain-circuit"
                    style="width:16px;height:16px;vertical-align:middle;color:var(--accent-primary);"></i>
                Agent Aptus actif — État : <span id="emotion-indicator"
                    style="font-weight:700; color:var(--accent-primary);">Chargement...</span>
            </div>
        <?php else: ?>
            <div style="color:var(--text-secondary); display:flex; align-items:center; gap:1rem;">
                ...
                <button onclick="showClassEmotions()" class="btn btn-sm"
                    style="background:linear-gradient(135deg,#6366f1,#8b5cf6); color:white; border:none; padding:4px 12px; border-radius:6px; cursor:pointer; font-weight:600; display:flex; align-items:center; gap:6px; box-shadow:0 2px 8px rgba(99,102,241,0.3);">
...
<script>
    ...
    <?php if ($role === 'tuteur'): ?>
        ...
        function updateCockpitUI(stats) {
            ...
            const chartCanvas = document.getElementById('emotionsChart');
            if (chartCanvas) {
                ...
                window.myChart = new Chart(ctx, {
                    ...
                    options: { cutout: '78%', plugins: { legend: { display: false } }, animation: false }
                });
            }
            ...
            fetch('ajax_handler.php', { method: 'POST', body: fd })
                ...
                .then(data => {
                    ...
                    if (container && data.success && data.data) {
                        container.innerHTML = `<p style="color:#a5b4fc; font-weight:700; margin-bottom:5px;">✦ ANALYSE IA</p><p style="margin:0; line-height:1.4;">${data.data.analyseGlobale || data.data.analyse_globale}</p>`;
                    }
                });
        }
    <?php endif; ?>
