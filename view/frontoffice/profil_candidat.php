...
<div style="display:grid;grid-template-columns:1fr 2fr;gap:var(--space-6);align-items:start;">
  ...
  <div
    style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:var(--space-8);text-align:center;">
    <div
      style="width:120px;height:120px;border-radius:50%;background:linear-gradient(135deg,var(--accent-primary),var(--accent-secondary));display:flex;align-items:center;justify-content:center;margin:0 auto var(--space-4);font-size:2.5rem;font-weight:700;color:#fff;">
      AK
    </div>
    ...
    <div
      style="border-top:1px solid var(--border-color);padding-top:var(--space-5);margin-top:var(--space-4);text-align:left;display:flex;flex-direction:column;gap:var(--space-3);">
  ...
  <div style="display:flex;flex-direction:column;gap:var(--space-6);">
    ...
    <div
      style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:var(--space-6);margin-bottom:var(--space-6);">
      <h3
        style="font-size:var(--fs-lg);font-weight:600;margin-bottom:var(--space-5);display:flex;align-items:center;gap:var(--space-2);">
      ...
      <?php if (!empty($mesBadges)): ?>
        <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(110px, 1fr));gap:var(--space-4);">
          <?php foreach ($mesBadges as $b):
            ...
            if (strpos($b['nom'], 'Débutant') !== false) {
              $color = '#10b981';
              $icon = 'award';
            } elseif (strpos($b['nom'], 'Intermédiaire') !== false) {
              $color = '#f59e0b';
              $icon = 'zap';
            } elseif (strpos($b['nom'], 'Expert') !== false) {
              $color = '#8b5cf6';
              $icon = 'crown';
            }
            ?>
            <div
              style="text-align:center;padding:var(--space-3);border:1px solid var(--border-color);border-radius:var(--radius-md);background:var(--bg-secondary);transition:all 0.3s;"
              onmouseover="this.style.transform='translateY(-4px)';this.style.borderColor='<?php echo $color; ?>'"
              onmouseout="this.style.transform='none';this.style.borderColor='var(--border-color)'">
              <div
                style="width:50px;height:50px;margin:0 auto var(--space-2);background:<?php echo $color; ?>15;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid <?php echo $color; ?>33;">
              ...
              <p style="font-size:0.6rem;color:var(--text-tertiary);">Obtenu le
                <?php echo date('d/m/Y', strtotime($b['date_obtention'])); ?></p>
            </div>
          <?php endforeach; ?>
        </div>
      <?php else: ?>
        <div
          style="padding:var(--space-8);background:var(--bg-secondary);border:1px dashed var(--border-color);border-radius:var(--radius-md);text-align:center;">
          ...
          <p class="text-xs" style="color:var(--text-tertiary);margin-top:0.5rem;">Complétez votre première formation pour
            débloquer un trophée !</p>
        </div>
      <?php endif; ?>
    </div>
    ...
    <!-- Personal Info -->
    <div
      style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:var(--space-6);">
      <h3
        style="font-size:var(--fs-lg);font-weight:600;margin-bottom:var(--space-5);display:flex;align-items:center;gap:var(--space-2);">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-4);">
        ...
        <div class="form-group" style="grid-column:1/-1;">
          ...
          <textarea class="textarea" rows="3"
            placeholder="Décrivez-vous en quelques mots...">Développeur passionné avec 3 ans d'expérience en React, Node.js et Python. Spécialisé dans le développement d'applications web performantes et scalables.</textarea>
        </div>
      </div>
    </div>
