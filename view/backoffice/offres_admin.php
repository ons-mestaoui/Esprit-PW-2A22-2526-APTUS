...
// Load entreprise list for the form dropdown
try {
    $db_admin = config::getConnexion();
    $entreprises_list = $db_admin->query(
        "SELECT u.id_utilisateur, u.nom FROM utilisateur u WHERE u.role = 'Entreprise' ORDER BY u.nom"
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $salaire = str_replace([' ', ','], ['', '.'], trim($_POST['salaire'] ?? ''));
...
<?php if ($action === 'list'): ?>
  <?php 
    ...
    // Compute real engagement & views stats
    try {
        $db_stats = config::getConnexion();
        $totalCands     = (int)$db_stats->query("SELECT COUNT(*) FROM candidatures")->fetchColumn();
        $totalOffresAll = (int)$db_stats->query("SELECT COUNT(*) FROM offreemploi")->fetchColumn();
        $offresEngaged  = (int)$db_stats->query("SELECT COUNT(DISTINCT id_offre) FROM candidatures")->fetchColumn();
        $engagementPct  = $totalOffresAll > 0 ? round($offresEngaged / $totalOffresAll * 100, 1) : 0;
        $vuesEstimees   = $totalCands * 7;
        $vuesFormatted  = $vuesEstimees >= 1000 ? round($vuesEstimees / 1000, 1) . 'k' : $vuesEstimees;
    } catch (Exception $e) {
        $engagementPct = 0;
        $vuesFormatted = '—';
    }
  ...
  <div class="grid grid-4 gap-6 mb-8 stagger">
    ...
    <div class="stat-card">
      <div>
        ...
        <div class="stat-card__value"><?php echo $engagementPct; ?>%</div>
        <div class="stat-card__trend up"><i data-lucide="trending-up" style="width:14px;height:14px;"></i> offres avec candidatures</div>
      </div>
      <div class="stat-card__icon teal"><i data-lucide="heart" style="width:22px;height:22px;"></i></div>
    </div>
    <div class="stat-card">
      <div>
        <div class="stat-card__label">Vues estimées</div>
        <div class="stat-card__value"><?php echo $vuesFormatted; ?></div>
        <div class="stat-card__trend up"><i data-lucide="trending-up" style="width:14px;height:14px;"></i> ~7× candidatures</div>
      </div>
                ...
                <section>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                        <div class="form-group">
                            <label class="form-label" style="font-weight: 600; margin-bottom: 0.6rem; display: block;">Entreprise <span style="color: #ef4444;">*</span></label>
                            <?php $currentEntreprise = (int)(isset($form_data['id_entreprise']) ? $form_data['id_entreprise'] : ($offreEdit['id_entreprise'] ?? 0)); ?>
                            <select name="id_entreprise" class="input <?php echo isset($errors['id_entreprise']) ? 'has-error' : ''; ?>" style="height: 52px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--bg-secondary); cursor: pointer;">
                                <option value="0">-- Sélectionner une entreprise --</option>
                                <?php foreach ($entreprises_list as $ent): ?>
                                    <option value="<?php echo $ent['id_utilisateur']; ?>" <?php echo $currentEntreprise === (int)$ent['id_utilisateur'] ? 'selected' : ''; ?>>
                                        <?php echo htmlspecialchars($ent['nom']); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <?php if (isset($errors['id_entreprise'])): ?>
                                <span style="color: #ef4444; font-size: 0.85rem; margin-top: 0.5rem; display: flex; align-items: center; gap: 0.4rem;"><i data-lucide="alert-circle" style="width:14px;height:14px;"></i> <?php echo $errors['id_entreprise']; ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="form-group">
