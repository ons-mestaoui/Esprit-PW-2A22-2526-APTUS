window.AIAgentUtils = {
    ...
    triggerFlashBriefing: async function(reportId, title) {
        ...
        try {
            const res = await fetch('/aptus_first_official_version/view/frontoffice/ajax_get_rapport.php?id=' + reportId);
            const data = await res.json();

            let contentText = "";
            if (data.success && data.contenu) {
                contentText = "\nVoici le contenu du rapport : " + data.contenu.substring(0, 1500); // Limit length to avoid token limits
            }

            // Construct the prompt for the AI
            const text = "Fais-moi un briefing audio très court (3 phrases maximum) du rapport intitulé: " + title + contentText;
            window.AIAssistant.processUserRequest(text);
        } catch (e) {
            console.error("Error fetching report content for briefing", e);
